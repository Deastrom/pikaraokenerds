...
async def fetch_karaokenerds_json(url, retries=3):
...
async def fetch_and_extract_yt_channel(pattern: ChannelPattern):
...
async def fetch_karaokenerds_songs(base_url, total_records, length, semaphore):
    ...
    for start in range(0, total_records, length):
        async with semaphore:
            tasks.append(fetch_karaokenerds_json(base_url + str(start)))
...
async def _scrape():
    ...
    initial_data = await fetch_karaokenerds_json(base_url + '0')
    ...
    channel_tasks = [fetch_and_extract_yt_channel(pattern) for pattern in CHANNEL_PATTERNS.values()]
